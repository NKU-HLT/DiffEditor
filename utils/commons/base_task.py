...
class BaseTask(nn.Module):
    ...
    def configure_optimizers(self):
        ...
        # self.scheduler = self.build_scheduler_cs(optm)
    ...
    def configure_optimizers_cs(self):
        optm1,optm2 = self.build_optimizer_cs(self.model)
        self.scheduler1, self.scheduler2= self.build_scheduler_cs(optm1,optm2)
    ...
    def on_after_optimization(self, epoch, batch_idx, optimizer, optimizer_idx):
        ...
        #     self.scheduler1.step(self.global_step // hparams['accumulate_grad_batches'])
        # if self.scheduler2 is not None:
        #     self.scheduler2.step(self.global_step // hparams['accumulate_grad_batches'])

    def on_after_optimization_cs(self, epoch, batch_idx, optimizer, optimizer_idx):
        # if self.scheduler is not None:
        #     self.scheduler.step(self.global_step // hparams['accumulate_grad_batches'])
        if self.scheduler1 is not None:
            self.scheduler1.step(self.global_step // hparams['accumulate_grad_batches'])
    ...
    def start(cls):
        ...
        print('---------------work_dir-----')
        print(work_dir)
        print("in basetask decide train/test")
