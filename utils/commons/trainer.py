...
class Trainer:
    ...
    def run_single_process(self, task):
        ...
        if not self.testing:
            if "use_reversal_classifier" in hparams and hparams["use_reversal_classifier"] == True:
                self.optimizers = task.configure_optimizers_cs()
            else:
        ...
        if self.use_ddp:
            self.task = self.configure_ddp(self.task,self.root_gpu)
            dist.barrier()
    ...
    def train(self):
        ...
        # 这里得到的要是zh和en的分别的dataloader
        
        
        # 这里得到的data_loader
        epoch = self.current_epoch
        # run all epochs
        if "frozen_steps" in hparams and "use_reversal_classifier" in hparams and hparams["frozen_steps"]>=0 and hparams["use_reversal_classifier"]==True:
            hparams["use_reversal_classifier"]=False
            # for param in task_ref.model.fs.reversal_classifier.parameters():
        ...
        while True:
            ...
            for batch_idx, batch in enumerate(train_pbar):
                ...
                train_pbar.set_postfix(**pbar_metrics,refresh=True) # 运行完这个之后又答应一行
                ...
                if self.global_step > self.max_updates:
                    print("| Training end..")
                    break
                if  "frozen_steps" in hparams and "use_reversal_classifier" in hparams and hparams["frozen_steps"]>0 and self.global_step > hparams["frozen_steps"] and hparams["use_reversal_classifier"]==False:
                    hparams["use_reversal_classifier"]=True
                    # 接下来并且把不是classifier的部分冻结
    ...
    def run_training_batch(self, batch_idx, batch):
        ...
        for opt_idx, optimizer in enumerate(self.optimizers):
            ...
            if self.print_nan_grads:
                ...
                for name, param in task_ref.named_parameters():
                    # print(name)
                    # model.fs.reversal_classifier._classifier.0.weight
                    # model.fs.reversal_classifier._classifier.0.bias
                    # model.fs.reversal_classifier._classifier.1.weight
                    # model.fs.reversal_classifier._classifier.1.bias
                    if name=="model.fs.reversal_classifier._classifier.0.weight":
                        print(name, param, param.grad/output['progress_bar']['lr_0'])
            ...
            if (self.global_step + 1) % self.accumulate_grad_batches == 0:
                ...
                optimizer.zero_grad()
                if "use_reversal_classifier" in hparams and hparams["use_reversal_classifier"] == True:
                    task_ref.on_after_optimization_cs(self.current_epoch, batch_idx, optimizer, opt_idx)
                else:
