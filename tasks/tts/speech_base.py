...
class SpeechBaseTask(BaseTask):
    ...
    def val_dataloader(self):
        if 'val_sets' in hparams and hparams['val_sets'] != '':
            val_sets = hparams['val_sets'].split("|")
            # check if all train_sets have the same spk map and dictionary
            binary_data_dir = hparams['binary_data_dir']
            file_to_cmp = ['phone_set.json']
            if os.path.exists(f'{binary_data_dir}/word_set.json'):
            ...
            if hparams['use_spk_id']:
                file_to_cmp.append('spk_map.json')
            for f in file_to_cmp:
                for ds_name in val_sets:
                    base_file = os.path.join(binary_data_dir, f)
                    ds_file = os.path.join(ds_name, f)
                    assert filecmp.cmp(base_file, ds_file), \
                        f'{f} in {ds_name} is not same with that in {binary_data_dir}.'
            valid_dataset = BaseConcatDataset([
                self.dataset_cls(prefix=hparams['valid_set_name'], shuffle=True, data_dir=ds_name) for ds_name in val_sets])
        else:       
    ...
    def build_scheduler(self, optimizer):
        if hparams['scheduler'] == 'rsqrt':
            ...
            #used for cs speech edit
            # return torch.optim.lr_scheduler.StepLR(
            #  optimizer=optimizer, step_size=10000, gamma=0.5)
            
        else:
            return NoneSchedule(optimizer, hparams['lr'])
        
        
        # for code_switched speech edit
    def build_scheduler_cs(self, optimizer1,optimizer2):
        # if hparams['scheduler'] == 'rsqrt':
        #     return RSQRTSchedule(optimizer, hparams['lr'], hparams['warmup_updates'], hparams['hidden_size'])
        # elif hparams['scheduler'] == 'warmup':
        #     return WarmupSchedule(optimizer, hparams['lr'], hparams['warmup_updates'])
        ...
        # return    WarmupSchedule(optimizer1, hparams['lr'], hparams['warmup_updates']),NoneSchedule(optimizer2, hparams['lr_classifier']) 
    # torch.optim.lr_scheduler.StepLR(
                # optimizer=optimizer2, step_size=10000, gamma=0.1)
                
        return torch.optim.lr_scheduler.StepLR(optimizer=optimizer1, step_size=50000, gamma=0.5), torch.optim.lr_scheduler.StepLR(optimizer=optimizer2, step_size=50000, gamma=0.5)
        # return  WarmupSchedule(optimizer1, hparams['lr'], hparams['warmup_updates']), WarmupSchedule(optimizer2, hparams['lr'], hparams['warmup_updates'])
    ...
    def build_optimizer_cs(self, model):
        all_params = model.named_parameters()

        # 选择性地包含或排除参数
        interested_params = []
        classifier_params = []
        exclude_params = ["reversal_classifier._classifier.0.weight",
                            "reversal_classifier._classifier.0.bias",
                            "reversal_classifier._classifier.1.weight",
                            "reversal_classifier._classifier.1.bias"]  # 你想要排除的参数列表

        for name, param in all_params:
            # print(name)
            if name not in exclude_params:
                interested_params.append(param)
            else:
        ...
        self.optimizer_classifier = optimizer2 = torch.optim.AdamW(
            classifier_params,
            lr=hparams['lr_classifier'],
            betas=(hparams['optimizer_adam_beta1'], hparams['optimizer_adam_beta2']),
            weight_decay=hparams['weight_decay'])
    ...
    def _training_step(self, sample, batch_idx, _):
        ...
        if 'use_reversal_classifier' in hparams and hparams['use_reversal_classifier']==True:
            loss_output['cla']=_['cla']
            
        if 'use_reversal_classifier_phoneme' in hparams and hparams['use_reversal_classifier_phoneme']==True:
            loss_output['cla_phoneme']=_['cla_phoneme']
