...
class FastSpeech(nn.Module):
    def __init__(self, dict_size, hparams, out_dims=None):
        ...
        if 'use_separate_encoder' in hparams and hparams['use_separate_encoder'] == True:
            self.encoder_zh = FS_ENCODERS[hparams['encoder_type']](hparams, dict_size)
            self.encoder_en = FS_ENCODERS[hparams['encoder_type']](hparams, dict_size)
        else:
        ...
        # self.word_out = nn.Linear(768, self.hidden_size, bias=True)
        self.pitch_proj=nn.Linear(self.hidden_size, self.hidden_size+word_dim, bias=True)
        self.spk_embed_proj_expand= nn.Linear(256, self.hidden_size+word_dim, bias=True)
    def forward(self, txt_tokens, time_mel_masks, mel2ph, spk_embed,
                f0, uv, spk_id=None, skip_decoder=True, infer=False, use_pred_mel2ph=False, use_pred_pitch=False,raw_txt=None,ph2word=None,bert_input=None,word2bert=None,word_token=None,**kwargs):
        ret = {}
   
        if 'use_separate_encoder' in self.hparams and  self.hparams['use_separate_encoder']== True:
        ...
        # print('word2bert')
        # print(word2bert[0])
        word_emb=self.word_encoder(**bert_input)
        # print()
        # print(word_emb['last_hidden_state'])
        word_emb = word_emb['last_hidden_state'].to(encoder_out.device)
 
        word_aggregated = torch.zeros(word_token.shape[0], word_token.shape[1], embedding_dim).to(encoder_out.device)
        expanded_word2bert = word2bert.unsqueeze(-1).expand(-1, -1, embedding_dim).to(encoder_out.device)
        word_aggregated.scatter_add_(1, expanded_word2bert, word_emb)

        word_emb_expaned=torch.zeros(ph2word.shape[0], ph2word.shape[1], embedding_dim).to(encoder_out.device)
        for i in range(ph2word.shape[0]):
            word_emb_expaned[i] = word_aggregated[i][ph2word[i]-1]
        
   
        
        # 然后concate到phoneme_emb上面去,放到return的时候加，不参与dur和pitch的计算

        
    
        # word_emb_expaned = self.word_out(word_emb_expaned)

        src_nonpadding = (txt_tokens > 0).float()[:, :, None]
        style_embed = self.forward_style_embed(spk_embed, spk_id)

        # add dur 这里的encoder_out就是phone——embedding
        word_emb_expaned=word_emb_expaned.to(encoder_out.device)
        # encoder_out = torch.cat((encoder_out,word_emb_expaned),dim=-1)
        # print(encoder_out.shape)
        dur_inp = (encoder_out + style_embed) * src_nonpadding
        
        
        # 如果use_pred_mel2ph=false 那么mel2ph 不变，
        # training的时候mel2ph不变
        ...
        if self.hparams['use_pitch_embed']:
            ...
            pitch_out= self.forward_pitch(pitch_inp, time_mel_masks, f0, uv, mel2ph, ret, use_pred_pitch=use_pred_pitch)
        ...
        pitch_out_expand=self.pitch_proj(pitch_out)
        style_embed_expanded = self.forward_style_embed_expand(spk_embed, spk_id)
        decoder_inp = (decoder_inp + style_embed_expanded+pitch_out_expand) * tgt_nonpadding
        # word_emb_expaned = expand_states(word_emb_expaned, mel2ph)
        
        # decoder_inp = torch.cat((decoder_inp,word_emb_expaned),dim=-1)
        
        ret['decoder_inp'] =decoder_inp
        ret['classifier_inp'] = decoder_inp * tgt_nonpadding
        if 'use_reversal_classifier_phoneme' in self.hparams and self.hparams['use_reversal_classifier_phoneme']:
            speaker_prediction_phoneme =self.reversal_classifier(encoder_out)
            ret['speaker_prediction_phoneme']=speaker_prediction_phoneme
    ...
    def forward_style_embed(self, spk_embed=None, spk_id=None):
        ...
        return style_embed
    def forward_style_embed_expand(self, spk_embed=None, spk_id=None):
        # add spk embed
        style_embed = 0
        if self.hparams['use_spk_embed']:
            style_embed = style_embed + self.spk_embed_proj_expand(spk_embed)[:, None, :]
        if self.hparams['use_spk_id']:
            style_embed = style_embed + self.spk_id_proj(spk_id)[:, None, :]
        return style_embed
    ...
    def forward_pitch(self, decoder_inp, time_mel_masks, f0, uv, mel2ph, ret, use_pred_pitch):
        ...
        # print(pitch_pred_inp.shape)
        # print(self.pitch_embed(masked_gt_pitch.long()).shape)
        pitch_pred_inp = pitch_pred_inp + self.pitch_embed(masked_gt_pitch.long())

        # Forward pitch
        # 如果要pred pitch那么输入是masked-uv和masked_f0
        if self.hparams['predictor_grad'] != 1:
